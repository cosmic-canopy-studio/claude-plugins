...
1. Create script (e.g., `autoloads/game_manager.gd`)
2. Project > Project Settings > Autoload tab
3. Enter path in "Path" field, set name (e.g., "GameManager")
4. Click "Add" button

### Via project.godot

Add directly to your `project.godot` file:
...
The `*` prefix means the autoload is enabled. Order matters for dependencies.
...
project/
├── autoloads/
│   ├── events.gd           # Global signals (no dependencies)
│   ├── save_manager.gd     # Persistence (no dependencies)
│   ├── audio_manager.gd    # Sound/music (no dependencies)
│   └── game_manager.gd     # Game state (may use others)
├── scenes/
├── scripts/
└── project.godot
```
...
- [Signals](signals.md) - Event Bus pattern uses signals for decoupled communication
- [Custom Resources](../data/resources.md) - For structured save data
