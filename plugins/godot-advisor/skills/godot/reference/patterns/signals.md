...
func _exit_tree() -> void:
    # Clean up ALL connections when removed from scene tree
    # Prevents errors if autoload emits after this node is freed
    if Events.player_health_changed.is_connected(_update_health_bar):
        Events.player_health_changed.disconnect(_update_health_bar)
    if Events.score_changed.is_connected(_update_score_display):
        Events.score_changed.disconnect(_update_score_display)
    if Events.player_died.is_connected(_show_game_over):
        Events.player_died.disconnect(_show_game_over)
```

### Why Cleanup Matters

When connecting to autoload signals (like EventBus), always disconnect in `_exit_tree()`:

1. **Autoloads persist** - They outlive your scene nodes
2. **Stale connections** - If autoload emits after node is freed, Godot errors
3. **Memory leaks** - Orphaned connections prevent garbage collection

For scene-local signals (parent-child), Godot auto-cleans connections when nodes are freed.
...
## Lambda Capture Behavior {#lambda-capture}

GDScript lambdas capture primitives **by value**, not reference:

```gdscript
# BAD: Primitive captured by value at lambda creation time
var count: int = 0
var callback := func() -> void:
    count += 1  # Modifies captured copy, not outer 'count'

signal.connect(callback)
signal.emit()
print(count)  # Still 0!

# GOOD: Use mutable container (array/dict) to capture changes
var counts: Array[int] = [0]
var callback := func() -> void:
    counts[0] += 1  # Modifies the array element correctly

signal.connect(callback)
signal.emit()
print(counts[0])  # Now 1
```

This is especially important when testing signals or tracking values across emissions.
...
- [Autoloads](autoloads.md) - Event Bus is typically an autoload singleton
- [Area Triggers](../physics/areas.md) - Areas emit body_entered/exited signals
