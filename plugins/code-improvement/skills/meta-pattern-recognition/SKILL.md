...
description: Spot patterns appearing in 3+ domains to find universal principles. USE WHEN same solution keeps working in different contexts, feeling déjà vu about a problem, multiple teams solving "different" problems the same way, suspecting a pattern exists but can't name it, looking for battle-tested solutions to new problem. (project) - same pattern 3+ domains, déjà vu problem-solving, universal principle, pattern across domains, reinventing wheel, haven't we done this before
when_to_use: when noticing the same pattern across 3+ different domains or experiencing déjà vu in problem-solving, same solution works in different contexts, suspecting universal principle exists
version: 1.1.0
---
