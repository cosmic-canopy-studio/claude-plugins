...
```
MUST attempt caption extraction before audio transcription
MUST include source URL and extraction method in output
MUST flag incomplete transcripts explicitly
NEVER deliver partial transcripts without warning
NEVER claim success without verifying transcript completeness
```

## Quality Score

Score = (Completeness × 40) + (Accuracy × 30) + (Format × 20) + (Metadata × 10)

- **Completeness**: Transcript covers full video duration
- **Accuracy**: No obvious errors in technical terms
- **Format**: Proper paragraph breaks, speaker labels
- **Metadata**: URL, method, timestamp included

Minimum acceptable: 80/100

## Validation Checklist

Before completing transcription work, verify:

- [ ] Video ID extracted and validated
- [ ] Transcription method selected (captions vs audio)
- [ ] Transcript is complete (matches video duration)
- [ ] Output formatted for intended use case
- [ ] Quality issues flagged if present
- [ ] Metadata included (URL, method, timestamp)